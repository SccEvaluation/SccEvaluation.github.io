...
if(isset($_GET['delete_prof'])){
    $delete = filter($_GET['delete_prof']);
    $ar = array("eu_id"=>$delete);
...
if(isset($_GET['delete_dean'])){
  $delete = filter($_GET['delete_dean']);
    $ar = array("eu_id"=>$delete);
    $tbl_name = "evaluation_user";
    $del = delete($dbcon,$tbl_name,$ar);
    if($del){
...
<hr>
<h4><i class="fa fa-book"></i> Evaluation Sheet Professor to Professor - <a href="eval_prof.php" class="btn btn-primary">Add Data</a></h4>
...
$student = $dbcon->query("SELECT * FROM evaluation_user 
  INNER JOIN user_account on user_account.user_id = evaluation_user.user_id WHERE user_role = '1'") or die(mysqli_error());
while($row = $student->fetch_assoc()):
?>
            ...
            <tr>
                ...
                <td class="center">
                    ...
                    <ul class="dropdown-menu" role="menu">
                      <li><a href="#" <?php echo 'onclick=" confirm(\'Are you sure you want to delete?\') 
      ?window.location = \'index.php?delete_prof='.$row['eu_id'].'\' : \'\';"'; ?>>Delete</a></li>
                    </ul>
                  </div>
                </td>
            </tr>
...
<hr>
  <h4><i class="fa fa-book"></i> Evaluation Sheet for Chairman to Chairman - <a href="eval_chairman.php" class="btn btn-primary">Add Data</a></h4>
   <table width="100%" class="table table-striped table-bordered table-hover" id="example3">
        <thead>
            <tr>
                <th>Evaluation Code</th>
                
                <th>Chairman Assigned</th>
                <th>Date Created</th>
                <th>Evaluation Status</th>
                <th>Option</th>
            </tr>
...
<tbody>
<?php 
$student = $dbcon->query("SELECT * FROM evaluation_user INNER JOIN user_account on user_account.user_id = evaluation_user.user_id WHERE user_role = '3'") or die(mysqli_error());
while($row = $student->fetch_assoc()):
?>
            ...
            <tr>
                ...
                <td class="center">
                    ...
                    <ul class="dropdown-menu" role="menu">
                      <li><a href="#" <?php echo 'onclick=" confirm(\'Are you sure you want to delete?\') 
      ?window.location = \'index.php?delete_dean='.$row['eu_id'].'\' : \'\';"'; ?>>Delete</a></li>
                    </ul>
                  </div>
                </td>
            </tr>
