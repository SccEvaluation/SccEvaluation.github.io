    ...
    <section style="padding:11px;">
      ...
      <div class="box box-solid" style="padding:15px;">
        ...
        <form method="post">
           ...
           <p></p>
           <div class="row">
            <div class="col-md-2">Professor:</div>
            <div class="col-md-9">
              <select class="form-control" name="prof_id">
                <?php 
                $query = "SELECT * FROM user_account WHERE user_role = '1'";
                $list = getdata_inner_join($query);
                ?>
                <?php if(!empty($list)):?>
                  <?php foreach ($list as $key => $value):?>
                    <option value="<?php echo $value->user_id?>">Professor Name: <?php echo $value->fname?> <?php echo $value->mname?> <?php echo $value->lname?> </option>
                  <?php endforeach;?>
                <?php else:?>
